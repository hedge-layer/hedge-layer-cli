...
function displayFeedResult(feed: FeedResult, globalOpts: GlobalOptions): void {
  if (globalOpts.json) {
    out.json(feed);
    return;
  }
  ...
  process.stdout.write(
    chalk.dim(`  ${feed.totalScanned.toLocaleString()} scanned · ${feed.totalAfterFilter.toLocaleString()} after filters\n\n`),
  );
